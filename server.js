...
// API: ลบออเดอร์ (admin)
app.delete('/api/admin/delete-order', async (req, res) => {
    try {
        ...
        if (!orderId) {
            return res.status(400).json({ error: 'ไม่ระบุ orderId' });
        }
        ...
        if (db) {
            ...
            try {
                ...
                if (order && order.slip) {
                    const slipPath = path.join(__dirname, order.slip);
                    if (fs.existsSync(slipPath)) {
                        fs.unlinkSync(slipPath);
                    }
                }
            } catch (slipError) {
                console.error('Error deleting slip file:', slipError);
            }
            ...
            // อ่านข้อมูลออเดอร์เพื่อลบไฟล์สลิป
            try {
                const orderData = fs.readFileSync(filePath);
